...
type Parser<'s, 'r> = Tuple<'s[], int>->Option<'r * int>
type Either<'l, 'r> = | Left of 'l | Right of 'r


let terminate (p:'s->Option<'r>): Parser<'s, 'r>= function  Tuple(s, i) -> match p s.[i] with | Some r -> Some (r, i+1) | None -> None
let terminal t = terminate (fun s -> if s = t then Some t else None)

//monadic
let ret r: Parser<'s, 'r> = function Tuple(s, i) -> Some (r, i)
let (>>=) (p:Parser<'s, 'a>) (f:'a->Parser<'s, 'b>) = function Tuple(s, i) -> p (Tuple(s, i)) |> Option.bind (fun (r, ri) -> (f r) (Tuple(s, ri)))
let (+>=) p f = p >>= (f >> ret)
